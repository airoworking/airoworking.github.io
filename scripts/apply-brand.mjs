...
async function decodeParts(parts, output, kind) {
  ...
  if (kind === 'avif') {
    if (binary.length < 16_000 || binary.subarray(4, 12).toString('ascii') !== 'ftypavif') throw new Error(`Invalid AVIF brand source for ${output}.`);
  } else if (kind === 'webp') {
    if (binary.length < 10_000 || binary.subarray(0, 4).toString('ascii') !== 'RIFF' || binary.subarray(8, 12).toString('ascii') !== 'WEBP') throw new Error(`Invalid WebP mascot source for ${output}.`);
  }
...
function heroHtml(html) {
  ...
  return `<section class="hero brand-hero" aria-labelledby="hero-title"><h1 id="hero-title" class="hero-sr-only">실무에 바로 쓰는 AI 자동화 가이드</h1><div class="brand-banner-image-wrap"><img class="brand-banner-image" src="./assets/brand/hero-banner-static.webp?v=20260904-static" width="2048" height="682" alt="실무에 바로 쓰는 AI 자동화. 업무 자동화, 비즈니스 자동화, AI 도구 활용법을 한눈에 소개하는 실무 가이드 배너" fetchpriority="high" decoding="sync"></div><div class="hero-copy"><div class="hero-actions"><a class="button button-primary" href="#audiences">분야별 가이드 보기</a><a class="button button-secondary" href="${secondaryHref}">${secondaryLabel}</a></div></div></section>`;
}
...
if (!html.includes('id="static-hero-lock"')) html = html.replace('</head>', '<style id="static-hero-lock">.brand-banner-image-wrap::after{display:none!important}.brand-banner-image-wrap:hover .brand-banner-image{transform:none!important;filter:none!important}.brand-banner-image{transform:none!important;filter:none!important;transition:none!important;animation:none!important}</style></head>');
...
console.log(`[brand] restored the uploaded high-quality static WebP banner, disabled hero motion, placed mascot as a small floating side guide, and applied ${Object.keys(icons).length} audience icons to ${target}`);
